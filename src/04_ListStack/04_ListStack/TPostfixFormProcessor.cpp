...
TPostfixFormProcessor::TokenType TPostfixFormProcessor::checkToken(const char token)
{
    if ((token == ' ') || (token == '\n') || (token == '\t') || (token == '\b'))
...
TPostfixFormProcessor::Priority TPostfixFormProcessor::checkPriority(const char first, const char second)
{
...
TPostfixFormProcessor::Variable TPostfixFormProcessor::findVariableByName(const Variables variables, const char name)
{
...
std::string TPostfixFormProcessor::findVariables(const std::string& expression)
{
...
bool TPostfixFormProcessor::checkExpression(const std::string& exp)
{
...
std::string TPostfixFormProcessor::parse(const std::string& expression)
{
    if (!checkExpression(expression))
        throw InvalidExpressionError();
    TStack<char>& postfixForm = *(this->postfixForm), &operations = *(this->operations);
    for (std::string::const_iterator token = expression.begin(); token != expression.end(); token++)
    {
        TokenType type = checkToken(*token);
...
bool TPostfixFormProcessor::test(const std::string& postfixForm)
{
    TStack<double> &calculated = *(this->calculated);
...
TPostfixFormProcessor::TPostfixFormProcessor(StackType stackType, size_t stackSize)
{
    if (stackType == StackType::Array)
    {
        operations = new TArrayStack<char>(stackSize);
        postfixForm = new TArrayStack<char>(stackSize);
        calculated = new TArrayStack<double>(stackSize);
    }
    else
    {
        operations = new TListStack<char>();
        postfixForm = new TListStack<char>();
        calculated = new TListStack<double>();
    }
}

double TPostfixFormProcessor::calculate(const std::string& postfixForm, const Variables variables)
{
    TStack<double>& calculated = *(this->calculated);
    for (std::string::const_iterator i = postfixForm.begin(); i != postfixForm.end(); i++)
    {
